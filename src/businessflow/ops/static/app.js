...
function renderKpis() {
  ...
  const overdueCount = accounts.filter((a) => a.flags.some((f) => f.label === "overdue")).length;
  ...
  const cards = [
    ...
    },
    {
      ...
      label: "Overdue",
      value: overdueCount,
      sub: overdueCount ? "past the grace period" : "nothing overdue",
    },
    {
      icon: iconInbox(),
...
document.getElementById("na-review-confirm-btn").addEventListener("click", async () => {
  ...
  try {
    ...
    $newAccountSuccess.innerHTML = `
      ...
      <button class="btn btn-approve" id="na-done-btn">Done</button>`;
